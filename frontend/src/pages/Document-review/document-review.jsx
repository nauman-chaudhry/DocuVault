...
export default function DocumentReviewPage() {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      try {
        const response = await fetchWithAuth(API_BASE_URL + '/docu/status/pending');
  ...
  const handleApprove = async () => {
    try {
      await updateDocumentStatus('Approved', comment);
  ...
  const updateDocumentStatus = async (newStatus, comment = '') => {
          ...
          {!currentDocument ? (
            ...
            <Stack spacing={3}>
              ...
              <Grid container spacing={2}>
                ...
                {currentDocument.submittedBy && (
                  <Grid item xs={6} sm={3}>
                    <Typography variant="caption" color="textSecondary">
                      Submitted By
                    </Typography>
                    <Typography variant="body1">{currentDocument.submittedBy}</Typography>
                  </Grid>
                )}
                {currentDocument.deadline && (
                  <Grid item xs={6} sm={3}>
                    ...
                    <Typography variant="body1" color={new Date(currentDocument.deadline) < new Date() ? 'error' : 'inherit'}>
                      {format(new Date(currentDocument.deadline), 'PPP')}
                    </Typography>
                  </Grid>
                )}
              </Grid>
