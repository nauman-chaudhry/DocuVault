...
function DocumentRow({ document }) {
  ...
  return (
    <Card
      sx={{
        mb: 2,
        border: isOverdue ? '1px solid' : '1px solid',
            ...
            <Stack direction="row" alignItems="center" spacing={1} flexWrap="wrap">
              ...
              {isOverdue && (
                <Chip
                  ...
                  label={`Overdue - pending ${daysSinceSubmission} days`}
                  color="error"
                  size="small"
                  variant="outlined"
                />
...
export default function DocumentTrackingPage() {
  ...
  const overdueCount = documents.filter(
    (doc) => doc.status === 'Pending' && differenceInDays(new Date(), new Date(doc.date)) > 7
  ).length;
